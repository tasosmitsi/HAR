...
DATA_PATH = Path('./intermediate_datafiles/')
DATASET_FNAME = 'our_set_5_result.csv'
...
target = 'acc_phone_x'
train_X, test_X, train_y, test_y = prepare.split_single_dataset_regression_by_time(dataset, target, '2021-06-02 08:12:37',
                                                                              '2021-06-02 08:49:25', '2021-06-02 09:10:44')
...
basic_features = ['acc_phone_y','acc_phone_z',
                  'labelOnTable','labelWashingHands','labelWalking','labelWalkingHoldingSmartphone','labelStreching','labelEating','labelRunning',
                  'light_phone_illuminance','prox_phone_distance']
...
dftest = adfuller(dataset[target], autolag='AIC')

# plt.Figure(); autocorrelation_plot(dataset[target])
# DataViz.save(plt)
# plt.show()
...
for i in range(0, len(possible_feature_sets)):
    ...
    if (feature_names[i] == 'initial set'):
        regr_train_y, regr_test_y = learner.time_series(selected_train_X, train_y, selected_test_X, test_y, gridsearch=False)
...
DataViz.plot_numerical_prediction_versus_real(train_X.index, train_y, regr_train_y[target], test_X.index, test_y, regr_test_y[target], 'heart rate')
